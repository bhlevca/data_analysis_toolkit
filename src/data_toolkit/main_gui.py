...
from .plugin_system import (PluginManager, get_example_plugins,
                            get_plugin_template)
...
# Optional neural networks import (requires TensorFlow)
try:
...
class AdvancedDataAnalysisGUI:
    ...
    def _create_tabs(self):
        ...
        self._create_ml_tab()
        self._create_neural_networks_tab()  # New neural networks tab
    ...
    def _create_ml_tab(self):
        ...
        # Get available models from MLModels class attributes
        models = list(MLModels.REGRESSION_MODELS.keys()) + list(MLModels.CLASSIFICATION_MODELS.keys())
    ...
    def _create_neural_networks_tab(self):
        """Tab for neural network models (MLP, LSTM, Autoencoder)"""
        tab = ttk.Frame(self.notebook)
        self.notebook.add(tab, text="🧠 Neural Networks")

        if not NEURAL_NETWORKS_AVAILABLE:
            # Show message if TensorFlow is not installed
            msg_frame = ttk.Frame(tab)
            msg_frame.pack(fill="both", expand=True, padx=20, pady=20)
            ttk.Label(msg_frame, text="⚠️ Neural Networks Unavailable",
                     style="Header.TLabel").pack(pady=10)
            ttk.Label(msg_frame,
                     text="TensorFlow is required for neural networks.\n\n"
                          "Install with: pip install tensorflow",
                     style="Info.TLabel", justify="center").pack(pady=10)
            return

        # Model type selection
        model_frame = ttk.LabelFrame(tab, text="Neural Network Model Selection")
        model_frame.pack(fill="x", padx=10, pady=10)

        # Neural network model types
        self.nn_model_var = tk.StringVar(value="MLP Regressor")
        nn_models = ["MLP Regressor", "MLP Classifier", "LSTM Forecast", "Autoencoder Anomaly"]

        ttk.Label(model_frame, text="Model Type:").pack(side="left", padx=10, pady=10)
        nn_dropdown = ttk.Combobox(model_frame, textvariable=self.nn_model_var,
                                   values=nn_models, state="readonly", width=25)
        nn_dropdown.pack(side="left", padx=5)

        ttk.Button(model_frame, text="Train Neural Network",
                  command=self.train_neural_network).pack(side="left", padx=10)

        # Parameters frame
        params_frame = ttk.LabelFrame(tab, text="Neural Network Parameters")
        ...
        ttk.Label(row1, text="Hidden Layers:").pack(side="left", padx=10)
        self.nn_hidden_layers = tk.StringVar(value="64,32")
        ttk.Entry(row1, textvariable=self.nn_hidden_layers, width=15).pack(side="left", padx=5)
        ttk.Label(row1, text="(comma-separated)").pack(side="left", padx=5)

        ttk.Label(row1, text="Epochs:").pack(side="left", padx=10)
        self.nn_epochs = tk.IntVar(value=100)
        ttk.Entry(row1, textvariable=self.nn_epochs, width=8).pack(side="left", padx=5)

        ttk.Label(row1, text="Batch Size:").pack(side="left", padx=10)
        self.nn_batch_size = tk.IntVar(value=32)
        ttk.Entry(row1, textvariable=self.nn_batch_size, width=8).pack(side="left", padx=5)

        # Row 2: LSTM-specific
        row2 = ttk.Frame(params_frame)
        row2.pack(fill="x", pady=5)

        ttk.Label(row2, text="LSTM Lookback:").pack(side="left", padx=10)
        self.nn_lookback = tk.IntVar(value=10)
        ttk.Entry(row2, textvariable=self.nn_lookback, width=8).pack(side="left", padx=5)

        ttk.Label(row2, text="Forecast Horizon:").pack(side="left", padx=10)
        self.nn_horizon = tk.IntVar(value=5)
        ttk.Entry(row2, textvariable=self.nn_horizon, width=8).pack(side="left", padx=5)

        ttk.Label(row2, text="Validation Split:").pack(side="left", padx=10)
        self.nn_val_split = tk.DoubleVar(value=0.2)
        ttk.Entry(row2, textvariable=self.nn_val_split, width=8).pack(side="left", padx=5)

        # Results
        results_frame = ttk.LabelFrame(tab, text="Neural Network Results")
        results_frame.pack(fill="both", expand=True, padx=10, pady=10)

        self.nn_results_text = tk.Text(results_frame, wrap="word",
                                       bg=ModernTheme.BG_MEDIUM, fg=ModernTheme.TEXT_PRIMARY,
                                       font=(ModernTheme.FONT_FAMILY, 10),
                                       borderwidth=0, padx=10, pady=10)
        nn_scroll = ttk.Scrollbar(results_frame, orient="vertical",
                                  command=self.nn_results_text.yview)
        self.nn_results_text.configure(yscrollcommand=nn_scroll.set)
        self.nn_results_text.pack(side="left", fill="both", expand=True)
        nn_scroll.pack(side="right", fill="y")
    ...
    def train_neural_network(self):
        """Train selected neural network model"""
        if not NEURAL_NETWORKS_AVAILABLE:
            messagebox.showerror("Error", "TensorFlow is not installed. Install with: pip install tensorflow")
            return
        ...
        model_type = self.nn_model_var.get()
        hidden_layers_str = self.nn_hidden_layers.get()
        epochs = self.nn_epochs.get()
        batch_size = self.nn_batch_size.get()
        validation_split = self.nn_val_split.get()
        lookback = self.nn_lookback.get()
        horizon = self.nn_horizon.get()

        # Parse hidden layers
        try:
            hidden_layers = [int(x.strip()) for x in hidden_layers_str.split(',')]
        except ValueError:
            messagebox.showerror("Error", "Hidden layers must be comma-separated integers (e.g., 64,32)")
            return

        # Initialize neural network models
        nn_models = NeuralNetworkModels(self.data_loader.df)

        self.nn_results_text.delete(1.0, tk.END)
        self.nn_results_text.insert(tk.END, f"═══ {model_type.upper()} TRAINING ═══\n\n")
        self.nn_results_text.insert(tk.END, "Training in progress... Please wait.\n\n")
        self.root.update()

        try:
            if model_type == "MLP Regressor":
                results = nn_models.mlp_regressor(
                    features=features,
                    target=target,
                ...
                self._display_mlp_results(results, "Regression")

            elif model_type == "MLP Classifier":
                results = nn_models.mlp_classifier(
                    features=features,
                    target=target,
                    hidden_layers=hidden_layers,
                    epochs=epochs,
                ...
                self._display_mlp_results(results, "Classification")

            elif model_type == "LSTM Forecast":
                if len(features) != 1:
                    messagebox.showwarning("Warning", "LSTM Forecast works best with a single feature. Using first feature.")

                results = nn_models.lstm_forecast(
                    column=features[0],
                    lookback=lookback,
                ...
                self._display_lstm_results(results)

            elif model_type == "Autoencoder Anomaly":
                results = nn_models.autoencoder_anomaly_detection(
                    features=features,
                    encoding_dim=min(hidden_layers) if hidden_layers else 8,
                    epochs=epochs,
                    batch_size=batch_size,
                    contamination=0.05
                )
            ...
            self.nn_results_text.delete(1.0, tk.END)
            self.nn_results_text.insert(tk.END, f"═══ ERROR ═══\n\n{str(e)}\n")
            messagebox.showerror("Training Error", str(e))

    def _display_mlp_results(self, results, model_type):
        """Display MLP training results"""
        self.nn_results_text.delete(1.0, tk.END)
        self.nn_results_text.insert(tk.END, f"═══ MLP {model_type.upper()} RESULTS ═══\n\n")

        if 'error' in results:
            self.nn_results_text.insert(tk.END, f"Error: {results['error']}\n")
            return

        # Architecture
        self.nn_results_text.insert(tk.END, "Architecture:\n")
        self.nn_results_text.insert(tk.END, f"  Hidden Layers: {results.get('hidden_layers', 'N/A')}\n")
        self.nn_results_text.insert(tk.END, f"  Input Features: {results.get('n_features', 'N/A')}\n\n")
        ...
        if history:
            ...
            self.nn_results_text.insert(tk.END, "Training History:\n")
            self.nn_results_text.insert(tk.END, f"  Final Training Loss: {final_loss:.4f}\n")
            if final_val_loss:
                self.nn_results_text.insert(tk.END, f"  Final Validation Loss: {final_val_loss:.4f}\n")

        # Metrics
        self.nn_results_text.insert(tk.END, "\nTest Metrics:\n")
        if model_type == "Classification":
            self.nn_results_text.insert(tk.END, f"  Accuracy: {results.get('accuracy', 0):.4f}\n")
            if 'classification_report' in results:
                self.nn_results_text.insert(tk.END, f"\n{results['classification_report']}\n")
        else:
            self.nn_results_text.insert(tk.END, f"  MSE: {results.get('mse', 0):.4f}\n")
            self.nn_results_text.insert(tk.END, f"  MAE: {results.get('mae', 0):.4f}\n")
            self.nn_results_text.insert(tk.END, f"  R² Score: {results.get('r2', 0):.4f}\n")
        ...
        if history and 'loss' in history:
            ...
            ax.set_xlabel('Epoch')
            ax.set_ylabel('Loss')
            ax.set_title(f'MLP {model_type} Training History')
            ax.legend()
            ax.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.show()

    def _display_lstm_results(self, results):
        """Display LSTM forecasting results"""
        self.nn_results_text.delete(1.0, tk.END)
        self.nn_results_text.insert(tk.END, "═══ LSTM FORECAST RESULTS ═══\n\n")
        ...
        # Architecture
        self.nn_results_text.insert(tk.END, "Configuration:\n")
        self.nn_results_text.insert(tk.END, f"  Lookback Period: {results.get('lookback', 'N/A')}\n")
        self.nn_results_text.insert(tk.END, f"  Forecast Horizon: {results.get('forecast_horizon', 'N/A')}\n\n")
        ...
        # Test metrics
        self.nn_results_text.insert(tk.END, "Test Metrics:\n")
        self.nn_results_text.insert(tk.END, f"  MSE: {results.get('test_mse', 0):.4f}\n")
        self.nn_results_text.insert(tk.END, f"  MAE: {results.get('test_mae', 0):.4f}\n")
        ...
        if len(forecast) > 0:
            ...
            for i, val in enumerate(forecast[:10], 1):
                self.nn_results_text.insert(tk.END, f"  Step {i}: {val:.4f}\n")
            if len(forecast) > 10:
                self.nn_results_text.insert(tk.END, f"  ... and {len(forecast)-10} more\n")

        # Plot forecast
        if 'actual' in results and 'predicted' in results:
            fig, ax = plt.subplots(figsize=(12, 5))
            ax.plot(results['actual'], label='Actual', alpha=0.7)
            ax.plot(results['predicted'], label='Predicted', alpha=0.7)
            if len(forecast) > 0:
                forecast_idx = range(len(results['actual']), len(results['actual']) + len(forecast))
                ax.plot(forecast_idx, forecast, 'r--', label='Future Forecast', linewidth=2)
    ...
    def _display_autoencoder_results(self, results):
        """Display Autoencoder anomaly detection results"""
        self.nn_results_text.delete(1.0, tk.END)
        self.nn_results_text.insert(tk.END, "═══ AUTOENCODER ANOMALY DETECTION ═══\n\n")
        ...
        # Architecture
        self.nn_results_text.insert(tk.END, "Architecture:\n")
        self.nn_results_text.insert(tk.END, f"  Encoding Dimension: {results.get('encoding_dim', 'N/A')}\n")
        self.nn_results_text.insert(tk.END, f"  Input Features: {results.get('n_features', 'N/A')}\n\n")
        ...
        if history:
            final_loss = history.get('loss', [0])[-1]
            self.nn_results_text.insert(tk.END, "Training History:\n")
            self.nn_results_text.insert(tk.END, f"  Final Reconstruction Loss: {final_loss:.6f}\n\n")

        # Anomaly detection
        n_anomalies = results.get('n_anomalies', 0)
        total = results.get('total_samples', 0)
        threshold = results.get('threshold', 0)
        self.nn_results_text.insert(tk.END, "Anomaly Detection:\n")
        self.nn_results_text.insert(tk.END, f"  Threshold: {threshold:.6f}\n")
        self.nn_results_text.insert(tk.END, f"  Anomalies Found: {n_anomalies} / {total}\n")
        self.nn_results_text.insert(tk.END, f"  Anomaly Rate: {100*n_anomalies/total:.2f}%\n")

        # Show anomaly indices
        anomaly_idx = results.get('anomaly_indices', [])
        if len(anomaly_idx) > 0:
            self.nn_results_text.insert(tk.END, f"\nAnomaly Indices (first 20):\n  {anomaly_idx[:20]}\n")

        # Plot reconstruction errors
        reconstruction_errors = results.get('reconstruction_errors', [])
        if len(reconstruction_errors) > 0:
            fig, ax = plt.subplots(figsize=(12, 5))
            ax.plot(reconstruction_errors, alpha=0.7, label='Reconstruction Error')
            ax.axhline(y=threshold, color='r', linestyle='--', label=f'Threshold ({threshold:.4f})')
            if len(anomaly_idx) > 0:
                ax.scatter(anomaly_idx, [reconstruction_errors[i] for i in anomaly_idx if i < len(reconstruction_errors)],
